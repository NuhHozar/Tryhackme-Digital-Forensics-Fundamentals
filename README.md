...
Task 1 - İntroducktion to Digital Forensic
...
The digital forensics team has various cases requiring different tools and techniques. However, the National Institute of Standards and Technology () defines a general process for every case. The works on defining frameworks for different areas of technology, including cyber security, where they introduce the process of digital forensics in four phases.
...
Email forensics: Email, the most common communication method between professionals, has become an important part of digital forensics. Emails are investigated to determine whether they are part of or fraudulent campaigns.
Answer the questions below
...
This creates a proper trail of evidence and helps preserve it. The document can be used to prove the and reliability of the evidence admitted in court. A sample can be downloaded from here (opens in new tab).
...
Autopsy: Autopsy (opens in new tab) is a popular open-source digital forensics platform. An investigator can import an acquired disk image into this tool, and the tool will conduct an extensive analysis of the image. It offers various features during image analysis, including keyword search, deleted file recovery, file metadata, extension mismatch detection, and many more.

The logo of Autopsy tool.

DumpIt: DumpIt (opens in new tab)offers the utility of taking a memory image from a Windows operating system. This tool creates memory images using a command-line interface and a few commands. The memory image can also be taken in different formats.

Volatility: Volatility (opens in new tab) is a powerful open-source tool for analyzing memory images. It offers some extremely useful plugins. Each artifact can be analyzed using a specific plugin. This tool supports various operating systems, including Windows, , macOS, and Android.
...
Firts we can use root@tryhackme:|~#cd /root/Rooms/introdigitalforensics then ls -la promt
